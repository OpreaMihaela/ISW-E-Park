...
function build_calendar($month, $year) {
    ...
    $stmt = $mysqli->prepare("SELECT * FROM bookings_record4 WHERE MONTH(DATE) = ? AND YEAR(DATE) = ?");
    ...
    $calendar.= "<a class='btn btn-xs btn-primary' href='?month=".date('m', mktime(0, 0, 0, 5, 1, $year))."&year=".date('Y', mktime(0, 0, 0, 5, 1, $year))."'>Luna urmatoare</a></center><br>";
     ...
     while ($currentDay <= $numberDays) {
         ...
         if($date<date('Y-m-d')){
             ...
             $calendar.="<td class='$today'><h4>$currentDay</h4> <a href='book4.php?date=".$date."' class='btn btn-success btn-xs'> <span class='glyphicon glyphicon-ok'></span>Rezerva Acum</a>";
         }
